  ...
  <div class="container">
    <div>
      ...
      <DataTable :value="FormData.Entity" :selection.sync="selectedRows" dataKey="id" class="p-datatable-gridlines">
        ...
        <Column field="RequestStatus_Name" header="Статус"></Column>
...
export default {
  ...
  methods: {
    Link(id) {
      return this.$router.push("/modules/request/RequestReestr/Request?id=" + id);
    },
    clickExit() {
      this.$router.push("/modules/request");
    },
    async clickNew() {
      const formData = {
        module: "request",
      ...
      if (answer.Error)
        ...
        return this.$router.push(
          "/modules/request/RequestReestr/Request?id=" + answer.DocMovement.id
        );
      }
    },
    async clickDelete() {
