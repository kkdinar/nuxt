      ...
      <DataTable :value="FormData.Entity" :selection.sync="selectedRows" dataKey="id" class="p-datatable-gridlines">
        ...
        <Column field="Subject" header="Тема">
          <template #body="slotProps">
            <Button :label="slotProps.data.Subject" class="p-button-link" @click="Link(slotProps.data.id)"/>
          </template>
        </Column>
...
export default {
  ...
  methods: {
    Link(id) {
      return this.$router.push("/modules/informing/InformReestr/Inform?id=" + id);
    },
    clickExit() {
      this.$router.push("/modules/informing");
    },
    async clickNew() {
      const formData = {
        module: "informing",
      ...
      if (answer.Error)
        ...
        return this.$router.push(
          "/modules/informing/InformReestr/Inform?id=" + answer.DocMovement.id
        );
      }
    },
    async clickDelete() {
